...
Easyaller can store a static IPv4 address, subnet mask, default gateway, and
one to three DNS server addresses in a profile. These are confidential but
...
address, a gateway in the same subnet, and one to three unique usable IPv4 DNS
server addresses. A profile never includes Wi-Fi secrets, VPN settings, or raw
commands.
...
and gateway, then sets the ordered DNS server list. It rereads the address and
DNS list before reporting success. On a command failure it attempts to restore
the previous IPv4 and DNS configuration on the same adapter.
