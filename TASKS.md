...
### [x] WP-080: Profile editor state and validation feedback

Added an explicit unsaved-change state based on a structural comparison between the saved profile and the current in-memory draft. Save and reset activate only for real changes; export, cloning, deletion, profile switching, refresh, opening the apply screen, and closing the main window are blocked until the operator saves or resets the draft. The same core validator now checks the draft without writing it, reports localized inline errors beside the affected profile, Windows, network, proxy, and application fields, and marks every collapsible section as ready, optional, or invalid. A live compact summary exposes the selected Windows editions, interface language, time zone, network, proxy, domain, and repeated application or instruction counts without opening every section. Static IPv4 and proxy-bypass inputs are enabled only when their corresponding modes can save those values.

Verified: `dotnet build Easyaller.slnx` with zero warnings and `dotnet test Easyaller.slnx --no-build` with 140 passing tests. Avalonia launched successfully on macOS, but its process was not exposed to the available accessibility application list, so the final layout still requires the planned Windows 11 visual pass.

### [x] WP-081: Searchable profile list
